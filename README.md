...
Eight [Model Context Protocol](https://modelcontextprotocol.io) servers that
...
# 2. install all MCPs from source (7 Python + 1 Rust)
make install
...
| **waveform-mcp-rs** | 7 | Rust-native VCD/FST parser · AXI4 decoder · RTL source mapping · **waveform debug** skill (Anthropic HTTP client built in) |
...
packages/
  ...
  mcp-servers/
    ...
    waveform-mcp-rs/           waveform_debug · VCD/FST · AXI4 · RTL mapping (Rust)
...
- 8 MCP servers at **v0.3.0** (Python 3.11 – 3.12, Rust stable)
- Mac + Win CI matrix across 10 Python packages + 1 Rust crate
...
- Tier 1 smoke (handshake + `list_tools`) on every MCP; Tier 2 pure-logic
  unit tests on the MCPs that have non-trivial in-process logic
